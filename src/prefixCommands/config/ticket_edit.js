...
export default {
    ...
    async execute(message, args) {
        ...
        if (sub === "add") {
            ...
            const style = ["primary", "secondary", "success", "danger"].includes(styleMaybe) ? styleMaybe : "primary";
            ...
            if (existing) return replyEmbed(message, { type: "error", title: "❌ Already Exists", description: "A category with that id already exists." });
        ...
        return replyEmbed(message, { type: "error", title: "❌ Usage", description: "`,ticket_edit title/text/add/remove/list`" });
    }
};
