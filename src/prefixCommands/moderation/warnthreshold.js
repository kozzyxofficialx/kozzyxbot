...
export default {
    ...
    async execute(message, args) {
        ...
        if (sub === "add") {
            ...
            if (exists) return replyEmbed(message, { type: "info", title: "ℹ️ Already Exists", description: `A threshold at **${count}** warnings already exists. Remove it first.` });
            settings.warnThresholds.push({ count, action, time });
            await saveSettings();
            return replyEmbed(message, { type: "settings", title: "✅ Threshold Added", description: `**${count}** warnings → **${action}**${action === "timeout" ? ` (${time}m)` : ""}.` });
        }
        ...
        if (sub === "remove") {
            ...
            await saveSettings();
            return replyEmbed(message, { type: "settings", title: "✅ Threshold Removed", description: `Removed threshold at **${count}** warnings.` });
        }
